    ...
    A: PageAccess,
{
    ...
    if cell_len != INTERIOR_CELL_PREFIX_SIZE {
        return Err(PageError::CorruptCell { slot_index, kind: CellCorruption::UnexpectedLength });
    }

    Ok(InteriorCellParts {
        cell_offset,
        left_child: format::read_u64(page.bytes(), cell_offset + CELL_LENGTH_SIZE),
        row_id: format::read_u64(page.bytes(), cell_offset + CELL_LENGTH_SIZE + PAGE_ID_SIZE),
    })
}

fn encoded_len() -> usize {
    INTERIOR_CELL_PREFIX_SIZE
}

fn write_cell(bytes: &mut [u8; PAGE_SIZE], cell_offset: usize, left_child: PageId, row_id: RowId) {
    format::write_u16(bytes, cell_offset, encoded_len() as u16);
    write_left_child(bytes, cell_offset, left_child);
    format::write_u64(bytes, cell_offset + CELL_LENGTH_SIZE + PAGE_ID_SIZE, row_id);
}
...
mod tests {
    ...
    #[test]
    fn rejects_interior_cell_with_short_length() {
        let mut bytes = new_interior_page(10);
        {
            let mut page = Page::<Write<'_>, Interior>::open(&mut bytes).unwrap();
            page.set_content_start((format::USABLE_SPACE_END - 4) as u16);
            format::write_u16(page.bytes_mut(), format::USABLE_SPACE_END - 4, 4);
            page.insert_slot(0, (format::USABLE_SPACE_END - 4) as u16).unwrap();
        }

        let err = Page::<Read<'_>, Interior>::open(&bytes).unwrap_err();
        assert_eq!(
            err,
            PageError::CorruptCell { slot_index: 0, kind: CellCorruption::UnexpectedLength }
        );
    }

    #[test]
    fn rejects_interior_cell_with_trailing_bytes() {
        let mut bytes = new_interior_page(10);
        {
            let mut page = Page::<Write<'_>, Interior>::open(&mut bytes).unwrap();
            let cell_offset = format::USABLE_SPACE_END - (INTERIOR_CELL_PREFIX_SIZE + 4);
            page.set_content_start(cell_offset as u16);
            format::write_u16(
                page.bytes_mut(),
                cell_offset,
                (INTERIOR_CELL_PREFIX_SIZE + 4) as u16,
            );
            page.insert_slot(0, cell_offset as u16).unwrap();
        }
    ...
    fn update_returns_not_found_for_missing_separator() {
        ...
        assert_eq!(err, PageError::KeyNotFound { key: 77 });
    }
    ...
    fn defragmentation_preserves_order_rightmost_child_and_footer() {
        ...
        let dead_offset = page.reserve_space_for_rewrite(INTERIOR_CELL_PREFIX_SIZE).unwrap();
